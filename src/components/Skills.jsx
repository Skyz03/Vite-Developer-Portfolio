...
function Skills() {
  ...
  return (
    <section className="relative max-w-6xl mx-auto px-6 py-16 text-white overflow-hidden">
      {/* Pattern image (bottom-right) */}
      <img
        src={patternImage}
        alt=""
        className="absolute bottom-0 right-0 w-40 sm:w-52 md:w-64 opacity-50 pointer-events-none select-none"
      />
      ...
      <div className="border-t border-gray-800 pt-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {items.map((it, idx) => (
            ...
            >
              <h3 className="text-3xl sm:text-4xl md:text-5xl font-extrabold tracking-tight">{it[0]}</h3>
        ...
        {/* Horizontal line on mobile only */}
        <hr className="border-gray-800 mt-10 md:hidden" />
      </div>
    </section>
  )
}
